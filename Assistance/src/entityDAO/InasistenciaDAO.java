...
public class InasistenciaDAO {
    ...
    public void Guardar(Inasistencia i){
        con = new ConexionDB();
        boolean rec = i.getRecupera()!=null;
        query = "Insert Into inasistencia (idpersona, fechaaviso, fechafalta, causa, aviso, recupera, idpersonareg) Values ('"+i.getPersona().getDni()+"','"
                 +i.getFechaaviso()+"','"+i.getFechafalta()+"','"+i.getCausa()+"','"+i.isAviso()+"','"+rec+"','"+i.getCarga().getDni()+"') returning id";
    	...
    	try{
            ...
            rs.next();
            i.setId(rs.getInt(1));
            System.out.println("La Carga de la Nueva Inasistencia fue Completada");
            con.close();
            st.close();}
    	catch(Exception e){
            System.out.println("Error al intentar guardar Inasistencia "+e.getMessage());}
    }
