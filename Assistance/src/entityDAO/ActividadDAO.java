...
public class ActividadDAO {
    ...
    public void Guardar(Actividad a, int idinasist){
        con = new ConexionDB();
        query = "Insert Into actividad (idinasist, descripcion, canthoras) Values ('"+idinasist+"','"+a.getDescripcion()+"','"
                                +a.getCanthoras()+"')";
    	...
    	try{
            ...
            st.execute(query);
            System.out.println("La Carga de la Nueva Actividad fue Completada");
            con.close();
            st.close();}
    	catch(Exception e){
            System.out.println("Error al intentar guardar Actividad "+e.getMessage());}
    }
